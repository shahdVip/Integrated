...
const Dashboard: React.FC<DashboardProps> = ({ user, onLogout, lang, toggleLang }) => {
  ...
  const [intensity, setIntensity] = useState<PumpIntensity>(PumpIntensity.MEDIUM);
  
  // State for Medications
  const [meds, setMeds] = useState<Medication[]>([
    { id: '1', name: 'Alprazolam', dosage: '0.25mg', schedule: '08:00', day: 1 },
    { id: '2', name: 'Lisinopril', dosage: '10mg', schedule: '21:00', day: 3 },
    { id: '3', name: 'Vitamin C', dosage: '500mg', schedule: '10:00', day: 0 },
  ]);
  const [newMed, setNewMed] = useState({ name: '', dosage: '', schedule: '', day: 0 });
  ...
  const getWeekDates = (offset: number) => {
    ...
    return Array.from({ length: 7 }, (_, i) => {
      ...
      return {
        dayNum: d.getDate(),
        month: d.toLocaleString(lang === Language.EN ? 'en-US' : 'ar-EG', { month: 'short' })
      };
    });
  };
        ...
        <header className="h-20 bg-white/80 backdrop-blur-xl border-b border-slate-200 px-10 flex items-center justify-between sticky top-0 z-40">
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-3 pr-6 border-r border-slate-200">
              ...
              <h1 className="text-xl font-black text-slate-900 tracking-tight">IMS Portal</h1>
            </div>
            <span className="text-slate-400 font-bold text-xs tracking-widest uppercase hidden sm:block">{t.dashboard}</span>
          </div>
          ...
          <div className="flex items-center gap-6">
            ...
            >
              <Globe size={18} />
              <span className="hidden sm:inline">{lang === Language.EN ? 'العربية' : 'English'}</span>
            </button>

            <div className={`flex items-center gap-4 ${lang === Language.EN ? 'pl-6 border-l' : 'pr-6 border-r'} border-slate-200`}>
              <div className="text-right hidden sm:block">
                <p className="text-sm font-black text-slate-900 leading-none">{user?.username || 'Guest User'}</p>
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">ID: #IMS-2025</p>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div className="lg:col-span-2 space-y-10">
              <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-200 relative overflow-hidden">
                <div className={`absolute top-0 ${lang === Language.EN ? 'right-0' : 'left-0'} p-10`}>
                  <div className={`px-5 py-2 rounded-full text-xs font-black tracking-widest uppercase flex items-center gap-2 ${
                    isPumping ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'
                  }`}>
                    <span className={`w-2.5 h-2.5 rounded-full ${isPumping ? 'bg-green-500 animate-ping' : 'bg-slate-300'}`}></span>
                ...
                <div className="flex flex-col md:flex-row items-center gap-16 pt-6">
                  <div className="flex flex-col items-center gap-6">
                    ...
                    >
                      <div className={`absolute inset-0 rounded-full border-8 border-white/20 scale-105 group-hover:scale-125 transition-transform ${isPumping ? 'animate-pulse' : ''}`}></div>
                      <Activity size={64} className={`text-white mb-3 ${isPumping ? 'animate-bounce' : ''}`} />
                      <span className="text-white font-black text-2xl tracking-tighter">
                        {isPumping ? t.stop : t.start}
                      </span>
                      <span className="text-white/70 font-black text-[10px] tracking-widest">{t.pumping}</span>
                    </button>
                  </div>
                  ...
                  <div className="flex-grow space-y-10 w-full">
                    ...
                    <div className="space-y-4">
                      <label className="text-xs font-black text-slate-400 uppercase tracking-widest">{t.intensity}</label>
                      <div className="flex gap-3 p-2 bg-slate-50 rounded-3xl border border-slate-100">
                        {[PumpIntensity.LOW, PumpIntensity.MEDIUM, PumpIntensity.HIGH].map((i) => (
                          <button
                            key={i}
                            onClick={() => setIntensity(i)}
                            className={`flex-1 py-4 rounded-2xl font-black text-sm transition-all ${
                              intensity === i ? 'bg-white text-blue-600 shadow-lg' : 'text-slate-400 hover:text-slate-600'
                            }`}
                          >
                            {i === PumpIntensity.LOW ? t.low : i === PumpIntensity.MEDIUM ? t.medium : t.high}
                          </button>
                        ))}
                      </div>
                    </div>
              ...
              <div className="bg-blue-600 p-8 rounded-[40px] shadow-2xl shadow-blue-200 text-white relative overflow-hidden group">
                ...
                <div className="space-y-4 relative z-10">
                  <div className="bg-white/10 p-4 rounded-2xl backdrop-blur-md border border-white/20">
                    <p className="text-[10px] text-blue-200 uppercase font-black tracking-widest mb-1">{lang === Language.EN ? 'Next Medication' : 'الدواء التالي'}</p>
          ...
          <div className="bg-white rounded-[50px] shadow-sm border border-slate-200 overflow-hidden">
            <div className="p-10 border-b border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-slate-50/30">
              <div className="flex items-center gap-5">
                ...
                <div>
                  <h2 className="text-3xl font-black text-slate-900 tracking-tighter">{t.healthSuite}</h2>
              ...
              <div className="flex items-center gap-4 bg-white p-2 rounded-2xl border border-slate-200 shadow-sm">
                 ...
                 >
                  {weekOffset === 0 ? (lang === Language.EN ? 'This Week' : 'هذا الأسبوع') : (lang === Language.EN ? 'Back to Today' : 'العودة لليوم')}
                 </button>
                 <button 
                  onClick={() => setWeekOffset(prev => prev + 1)}
                  className="p-2 hover:bg-slate-100 rounded-xl transition-colors"
              ...
              <div className="grid grid-cols-7 gap-6 min-w-[1000px]">
                {days.map((day, idx) => (
                  <div key={idx} className="flex flex-col gap-4">
                    <div className={`flex flex-col items-center border-2 rounded-3xl p-4 shadow-sm group transition-colors ${weekOffset === 0 && (new Date().getDay() === (idx === 6 ? 0 : idx + 1)) ? 'border-blue-500 bg-blue-50/20' : 'border-slate-100 bg-white hover:border-blue-300'}`}>
                      <span className="font-black text-slate-400 text-[10px] uppercase tracking-widest">{day}</span>
                      <span className="text-2xl font-black text-slate-800 mt-1">{weekDates[idx].dayNum}</span>
                      <span className="text-[10px] font-bold text-blue-500 uppercase">{weekDates[idx].month}</span>
                    </div>
                    <div className={`min-h-[320px] p-4 bg-slate-50/30 rounded-[32px] border-2 border-dashed transition-all duration-300 ${meds.some(m => m.day === idx) ? 'border-blue-200 bg-blue-50/10' : 'border-slate-100'} space-y-4`}>
                      {meds.filter(m => m.day === idx).map(m => (
                        <div key={m.id} className="p-5 bg-white border border-slate-200 rounded-2xl shadow-lg shadow-slate-100/50 group relative hover:-translate-y-1 transition-all animate-in zoom-in-95">
                          <button 
                          ...
                          <p className="font-black text-sm text-slate-800 leading-tight">{m.name}</p>
                          <p className="text-xs text-slate-400 mt-1 font-bold">{m.dosage}</p>
                      ...
                      {meds.filter(m => m.day === idx).length === 0 && (
                        <div className="h-full flex flex-col items-center justify-center text-slate-200 py-10">
                          ...
                          <span className="text-[10px] font-bold uppercase tracking-widest opacity-20">{lang === Language.EN ? 'Schedule' : 'جدول'}</span>
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <div className="p-10 bg-slate-50 border-t border-slate-100">
              <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 items-end">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.dosage}</label>
                  <input 
                    value={newMed.dosage} 
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{lang === Language.EN ? 'Week Day' : 'يوم الأسبوع'}</label>
                  <select 
                    value={newMed.day} 
                    onChange={e => setNewMed({...newMed, day: parseInt(e.target.value)})} 
